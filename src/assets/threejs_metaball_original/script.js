...
function updateStory(x, y, radius, merges, fps) {
    ...
    if (storyText) {
        const newText = getStoryText(x.toFixed(2), y.toFixed(2), radius.toFixed(2), merges, fps || 0);
...
function setupUI() {
    ...
    pane.addBinding(settings, 'preset', {
        options: {
            Moody: 'moody',
            Cosmic: 'cosmic',
            Minimal: 'minimal',
    ...
    mouseFolder.addBinding(settings, 'mouseProximityEffect').on('change', (ev) => {
